...
- (void)configLayout{
    ...
    if (WIDTH(self.uncheckedImagesView) != SELFWIDTH ||
        ...
        for (NSInteger i = 0; i < STARCOUNT; i++) {
            ...
            CGRect imageFrame = CGRectMake((starSize.width + SPACING) * i, (self.height-starSize.height)/2-2, starSize.width, starSize.height);
...
- (void)updateStarView:(CGFloat)ratio{
    ...
    switch (self.type) {
        ...
        case RatingTypeUnlimited:
            ...
            width = starSize.width * ratio + (SPACING * floor(ratio));
