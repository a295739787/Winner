...
+ (BOOL)isBlankString:(NSString *)str{
    ...
    if (strss == nil || strss == NULL){
        return YES;
    }
    if ([strss isKindOfClass:[NSNull class]]) {
        return YES;
    }
    if ([strss isEqual:@"<null>"]) {
        return YES;
    }
    if ([strss isEqual:@"[null]"]) {
        return YES;
    ...
    if ([[strss stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] length]==0) {
        return YES;
    }
    return NO;
}
...
+(NSString *)getAppInfo{
    ...
    deviceModel =FORMAT(@"%@,%@,%@,%@,%@,%@,%@,%@",bundleIdentifier,version,build,appName,deviceName,systemName,systemVersion,deviceUUID);
    return deviceModel;
}
...
{
    NSMutableArray *ary = [NSMutableArray arrayWithCapacity:42];
    for (int i=0; i<42; i++)
    {
        NSString *imageNamed = [[NSString alloc]initWithFormat:@"star_000%d",i];
    ...
    for (int i = 'A'; i <= 'Z'; i++) {
        ...
        if ([array count]) {
            [array sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
                NSString *word1 = obj1[@"nickname"];
                NSString *word2 = obj2[@"nickname"];
