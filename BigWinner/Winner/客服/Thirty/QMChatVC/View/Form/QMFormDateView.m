...
-(void)hideAnimateWithSelectedStatus:(BOOL)status {
    ...
    [UIView animateWithDuration:0.25f delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        CGRect re = self.frame;
        re.origin.y = re.size.height - contentHeiht;
        self_weak_.backgroudContentView.frame = re;
        self_weak_.alpha = .0f;
    } completion:^(BOOL finished) {
        if (status) {
            if (self_weak_.finishedBlock) {
                self_weak_.finishedBlock([self dateToString:self.datePicker.date format:(self.dateFormat.length) ? self.dateFormat : defaultDateFormat]);
                self_weak_.finishedBlock = nil;
            }
        }else{
            if (self_weak_.cancelBlock) {
                self_weak_.cancelBlock();
                self_weak_.cancelBlock = nil;
            }
        }
        [self_weak_ removeFromSuperview];
        [self_weak_.datePicker removeFromSuperview];
        self_weak_.datePicker = nil;
    }];
}
