...
- (void)viewDidAppear:(BOOL)animated {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [JXUIKit showWithString:@"当前是机器人为你服务"];
    });
...
- (void)changeUserInfaceStyle {
    ...
    self.associationView.backgroundColor = [UIColor colorWithHexString:isDarkStyle ? QMColor_Nav_Bg_Dark : QMColor_Nav_Bg_Light];
...
- (void)createUI {
    ...
    _navHeight = 0;
    // 坐席信息提示
    _titleView = [[QMChatTitleView alloc] initWithFrame: CGRectMake(0, SCREEN_top, 150, 40)];
    ...
    self.customNavBar.title = @"大赢家APP客服为您服务";
    WS(weakself);
    ...
    // 消息列表
    self.chatTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,0, QM_kScreenWidth, QM_kScreenHeight-kInputViewHeight) style:UITableViewStylePlain];
    ...
    NSLog(@" self.bottomView  == %@", self.bottomView );
    //录音动画面板
...
- (void)insertCardInfoMessage {
    ...
    [dic setObject:tempimage[0] forKey:@"cardImage"]; //此参数需要填写的是URL
    [dic setObject: _model.name forKey:@"cardHeader"];
    [dic setObject: _model.name forKey:@"label"];
    [dic setObject:FORMAT(@"销量%@",_model.realSalesVolume) forKey:@"cardSubhead"];
    [dic setObject:FORMAT(@"￥%.2f",_model.salesPrice.floatValue) forKey:@"cardPrice"];
...
- (void)beginNewChat:(BOOL)otherAgent {
    ...
    NSDictionary *param = @{@"customField":@{@"扩展信息key":@"扩展信息value",@"user_labels":@{@"vip":@"true",@"city":@"beijing"}}};
...
- (void)changeChatStatus:(BOOL)remark {
    ...
    NSArray *bottomArr = [QMConnect xbotBottomList:@"常用问题"];

    if (bottomArr.count > 0) {
        isBottomShow = YES;
        self.bottomView.hidden = NO;
//        self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-kInputViewHeight );
//        self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, kInputViewHeight );
        self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-kInputViewHeight-SCREEN_top-Heights);
...
- (void)changeBottomViewFrame {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.bottomView.hidden = NO;
        if (!self.chatInputView.inputView.isFirstResponder) {
            self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-kInputViewHeight-self->_navHeight-Heights);
        } else {
            self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, self.view.frame.size.height - self->keyBoardFrame.size.height - kInputViewHeight-Heights);
        }
    });
}
...
//        NSLog(@"--------------isShowAssociatsWithAgent");
...
//    NSLog(@"--------------------123");
    ...
    if (formView) {
        NSLog(@"QMChatFormView - QMChatFormView - QMChatFormView");
        return;
    }
    ...
    if (ceil(newFrame.origin.y) == [UIScreen mainScreen].bounds.size.height) {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            if (self->isBottomShow) {
                self.bottomView.hidden = YES;
//                self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-self->_navHeight-kInputViewHeight);
                self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-self->_navHeight-kInputViewHeight-Heights);
              
                self.bottomView.frame = CGRectMake(0, QM_kScreenHeight-kInputViewHeight-self->_navHeight-52, QM_kScreenWidth, 52);
//                [JXUIKit showErrorWithStatus:FORMAT(@"谈下去 == %.2f",self.chatTableView.height)];

            }else{
                self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-self->_navHeight-kInputViewHeight-Heights);
                self.bottomView.hidden = YES;
//                [JXUIKit showErrorWithStatus:FORMAT(@"self.chatTableView.height == %.2f",self.chatTableView.height)];
        ...
        [UIView animateWithDuration:0.3 animations:^{
            if (!self->keyboardIsShow) {
                ...
                if (self->isBottomShow) {
                    self.bottomView.hidden = YES;
                    self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, [UIScreen mainScreen].bounds.size.height-self->_navHeight-kInputViewHeight-Heights);
...
- (void)changeButtonStatus:(BOOL)down {
    if (down == YES) {
        ...
        [self.chatInputView.RecordBtn setTintColor:color];
    }else {
...
//    NSLog(@"identifier = %@",identifier);
    ...
    if ([cell isKindOfClass:[QMChatLogistcsInfoCell class]]) {
        ...
        lcell.showMore = ^{
            [weakSelf showMoreView:dataA];
        };
    }
    
    [cell setData:message avater:[UserModel sharedUserInfo].headIcon];
...
//    NSLog(@"-----------hengshuping");
    ...
    if (isBottomShow) {
        self.bottomView.hidden = NO;
        self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-_navHeight-kInputViewHeight-152);
        self.bottomView.frame = CGRectMake(0, QM_kScreenHeight-kInputViewHeight-_navHeight-52, QM_kScreenWidth, 52);
    }else{
        self.bottomView.hidden = NO;
        self.bottomView.frame = CGRectMake(0, QM_kScreenHeight-kInputViewHeight-_navHeight-52, QM_kScreenWidth, 52);

        self.chatTableView.frame = CGRectMake(0, SCREEN_top, QM_kScreenWidth, QM_kScreenHeight-_navHeight-kInputViewHeight);
    }
