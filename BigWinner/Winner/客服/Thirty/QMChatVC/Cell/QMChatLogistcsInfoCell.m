...
- (void)setData:(CustomMessage *)message avater:(NSString *)avater {
    ...
    if ([message.fromType isEqualToString:@"0"]) {
        //发送者
//        self.chatBackgroundView.image = [UIImage imageNamed:@"SenderCardNodeBkg"];
        ...
        //接收方
//        self.chatBackgroundView.image = [UIImage imageNamed:@"ReceiverTextNodeBkg"];
        [self.chatBackgroundView mas_updateConstraints:^(MASConstraintMaker *make) {
...
//    self.chatBackgroundView.image = [self.chatBackgroundView.image stretchableImageWithLeftCapWidth:20 topCapHeight:20];
...
+ (CGFloat)getCellHeight:(NSString *)title {
    ...
    CGFloat h = [title boundingRectWithSize:CGSizeMake(255*kScale6, 300) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont fontWithName:QM_PingFangSC_Reg size:14]} context:nil].size.height;
    height += h;
    return height;
}
