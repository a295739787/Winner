...
- (void)createUI {
    ...
    [self.iconImage sd_setImageWithUrlString:[UserModel sharedUserInfo].avatar];
...
- (void)setData:(CustomMessage *)message avater:(NSString *)avater {
    ...
    if ([message.fromType isEqualToString:@"0"]) {
        if(![avater containsString:@"http"]){
            avater = FORMAT(@"%@%@",API_IMAGEHOST,avater);
        }
        NSLog(@"avater == %@",avater);
//        if ([NSURL URLWithString:avater]) {
//            [self.iconImage sd_setImageWithURL:[NSURL URLWithString:avater] placeholderImage:[UIImage imageNamed:@"headimages"]];
//        }else {
//            self.iconImage.image = [UIImage imageNamed:@"headimages"];
//        }
        [self.iconImage sd_setImageWithUrlString:avater placeholderImage:[UIImage imageNamed:@"headimages"]];
