...
-(void)setModelBig:(GoodsModel *)modelBig{
    _modelBig = modelBig;
}
...
-(float)initTypeView
{
    if(_modelBig.status == RoleStatusStockPeisong){
        ...
        float upX = 10;
        float upY = 0;
        NSArray *strings = [_modelBig.specsValName componentsSeparatedByString:@";"];
        for (int i = 0; i<_model.typeArray.count; i++) {
            NSDictionary *dics = _model.typeArray[i];
            
            UIButton *btn= [JXUIKit buttonWithBackgroundColor:KLightGrayLinecol titleColorForNormal:[UIColor blackColor] titleForNormal:dics[@"name"] titleForSelete:dics[@"name"] titleColorForSelete:WhiteColor fontSize:13 font:nil];
            [btn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
            
            NSDictionary *dic = [NSDictionary dictionaryWithObject:btn.titleLabel.font forKey:NSFontAttributeName];
            CGSize size = [dics[@"name"] sizeWithAttributes:dic];
            //NSLog(@"%f",size.height);
            ...
            btn.frame = CGRectMake(upX, upY, size.width+30,CGFloatBasedI375(30));
            [JXUIKit ViewcornerRadius:CGFloatBasedI375(15) andColor:WhiteColor andWidth:0 :btn];
...
//                NSLog(@"str == %@  name == %@",str,dics[@"name"]);
//                if([str isEqual:dics[@"name"]]){
//                    btn.selected = YES;
//                    btn.backgroundColor = KBtncol;
//                }
//            }
            if([strings containsObject:dics[@"name"]]){
                btn.selected = YES;
                btn.backgroundColor = KBtncol;
