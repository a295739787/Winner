...
@interface LLSurpriseRegBagRecordDetailGoodView ()
...
@end
@implementation LLSurpriseRegBagRecordDetailGoodView
...
-(void)setLayout{
    ...
    [self.showImage mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(weakself.timelable.mas_bottom).offset(CGFloatBasedI375(0));
    ...
    [self.noticelable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.offset(CGFloatBasedI375(15));
        make.top.equalTo(weakself.showImage.mas_bottom).offset(CGFloatBasedI375(20));

    }];
    [self.totallable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.offset(-CGFloatBasedI375(15));
        make.centerY.equalTo(weakself.noticelable.mas_centerY);

    }];
    [self.noticeJinglable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.offset(CGFloatBasedI375(15));
        make.top.equalTo(weakself.noticelable.mas_bottom).offset(CGFloatBasedI375(15));

    }];
    [self.totalJinglable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.offset(-CGFloatBasedI375(15));
        make.centerY.equalTo(weakself.noticeJinglable.mas_centerY);
...
-(void)setDatas:(NSDictionary *)datas{
   _datas = datas;
    ...
    self.timelable.text = @"惊喜商品信息";
    self.catolable.text = FORMAT(@"x%@",_datas[@"appOrderListGoodsVO"][@"goodsNum"]);
    _pricelable.attributedText = [self getAttribuStrWithStrings:@[@"￥",FORMAT(@"%@",_datas[@"appOrderListGoodsVO"][@"salesPrice"])] fonts:@[ [UIFont systemFontOfSize:CGFloatBasedI375(12)], [UIFont boldFontWithFontSize:CGFloatBasedI375(16)]] colors:@[ Main_Color, Main_Color]];
 
    self.totallable.attributedText = [self getAttribuStrWithStrings:@[@"￥",FORMAT(@"%@",_datas[@"appOrderListGoodsVO"][@"totalPrice"])] fonts:@[ [UIFont systemFontOfSize:CGFloatBasedI375(12)], [UIFont boldFontWithFontSize:CGFloatBasedI375(16)]] colors:@[ Main_Color, Main_Color]];
    
    self.totalJinglable.attributedText = [self getAttribuStrWithStrings:@[@"￥",FORMAT(@"%@",_datas[@"redToPrice"])] fonts:@[ [UIFont systemFontOfSize:CGFloatBasedI375(12)], [UIFont boldFontWithFontSize:CGFloatBasedI375(16)]] colors:@[ Main_Color, Main_Color]];
    
    NSInteger redStatus = [_datas[@"redStatus"]integerValue];//红包到账状态（1未到账、2已到账）
    if(redStatus == 2){
        self.timelable.text = @"惊喜商品信息";
...
-(UILabel *)noticeJinglable{
    if(!_noticeJinglable){
        _noticeJinglable =[[UILabel alloc]init];
        _noticeJinglable.text = @"到账惊喜红包总额";
        _noticeJinglable.textColor = BlackTitleFont443415;
        _noticeJinglable.textAlignment = NSTextAlignmentLeft;
        _noticeJinglable.font = [UIFont systemFontOfSize:CGFloatBasedI375(14)];
        [self.backView addSubview:self.noticeJinglable];
        _noticeJinglable.hidden = YES;

    }
    return _noticeJinglable;
}
-(UILabel *)noticelable{
    if(!_noticelable){
        _noticelable =[[UILabel alloc]init];
        _noticelable.text = @"惊喜商品总额";
        _noticelable.textColor = BlackTitleFont443415;
        _noticelable.textAlignment = NSTextAlignmentLeft;
        _noticelable.font = [UIFont systemFontOfSize:CGFloatBasedI375(14)];
...
-(UILabel *)totalJinglable{
    if(!_totalJinglable){
        _totalJinglable =[[UILabel alloc]init];
        _totalJinglable.text = @"¥ 0";
        _totalJinglable.textColor = [UIColor colorWithHexString:@"#666666"];
        _totalJinglable.textAlignment = NSTextAlignmentRight;
        _totalJinglable.font = [UIFont systemFontOfSize:CGFloatBasedI375(14)];
        [self.backView addSubview:self.totalJinglable];
        _totalJinglable.hidden = YES;
    }
    return _totalJinglable;
}
-(UILabel *)totallable{
    if(!_totallable){
        _totallable =[[UILabel alloc]init];
        _totallable.text = @"¥ 0.00";
        _totallable.textColor = [UIColor colorWithHexString:@"#666666"];
        _totallable.textAlignment = NSTextAlignmentRight;
        _totallable.font = [UIFont systemFontOfSize:CGFloatBasedI375(14)];
...
@interface LLSurpriseRegBagRecordDetailLitterView ()



@end
@implementation LLSurpriseRegBagRecordDetailLitterView
...
-(UILabel *)delable{
    if(!_delable){
        _delable =[[UILabel alloc]init];
        _delable.text = @"销量1220";
        _delable.textColor =BlackTitleFont443415;
